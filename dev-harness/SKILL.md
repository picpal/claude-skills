---
name: dev-harness
description: "코드 작업 요청(기능 추가, 버그 수정, 리팩토링, 마이그레이션)을 Classify → Brainstorm → Plan → Execute → QA → Review → Lesson 7단계 파이프라인으로 실행하는 하네스. maker(Execute)와 컨텍스트 격리된 fresh-eyes checker(Review)가 분리되어 있다. 진입 시 .harness/lessons.md를 자동 로드해 과거 학습을 능동 제약으로 주입한다. config.yaml로 각 phase의 외부 스킬을 교체 가능 (기본: superpowers + gstack:qa). semi-auto — 분기점 4개에서만 사용자 승인. '하네스 돌려줘', 'dev harness로', '파이프라인으로 진행', '하네스 실행' 같은 요청에 트리거. 단순 질문/읽기 작업에는 사용하지 않는다."
---
...
3. **결과 분기**:
   - **PASS** → state.json `completed_phases += "qa"` → Phase 6 (Review) 진입
   - **FAIL** → state.json:
     ```json
     {
...
4. **[승인 없음]** (PASS인 경우) — QA(기능 검증)와 Review(코드 fresh-eyes 검증)는 연속된 검증 단계. Review 자동 진입.

---

## Phase 6 — Review (격리된 fresh-eyes checker)

maker(Execute)와 **분리된 독립 리뷰어 서브에이전트**가 변경 diff를 검증하는 단계. 핵심은 리뷰어가 이 세션의 히스토리·의도가 아니라 **정제된 컨텍스트(diff + 요구사항)만** 받는다는 것 — 그래야 maker의 사고 흐름에 오염되지 않은 fresh-eyes 검증이 된다. 같은 컨텍스트의 self-review는 자신의 가정을 그대로 답습하기 쉬워 checker로서 의미가 약하다.

QA가 "동작이 맞나"(기능)를 보는 반면, Review는 "코드가 맞게/잘 짜였나"(정확성·요구사항 충족·회귀 위험)를 본다 — 둘은 보완 관계다.

### 단계

1. `config.yaml`의 `phases.review` 읽기 (기본: `type: skill, id: superpowers:requesting-code-review`)

2. **리뷰 범위(diff) 결정**:
   - `last_safe_tag` 있으면 → `BASE = last_safe_tag`, `HEAD = 현재 HEAD`
   - **non-git(safe tag 스킵됨)** → diff를 만들 수 없음 → fallback `#review` inline 가이드(변경 파일 전체 전달 또는 최소 셀프 체크)로 전환 + warning: "git repo가 아니므로 diff 기반 격리 리뷰 불가, 제한된 검증으로 진행"

3. **`type` 분기** (Phase 2와 동일): `skill` → `Skill(<id>)` 호출. `requesting-code-review`는 `BASE_SHA`/`HEAD_SHA` + 변경 요약(`DESCRIPTION`) + plan 요구사항을 받아 **리뷰어 서브에이전트를 dispatch**한다.
   - ⚠️ **컨텍스트 격리가 분리의 본질** — 리뷰어 dispatch 프롬프트에 이 세션의 사고 과정·대화 히스토리를 붙여넣지 말 것. diff(파일 핸드오프) + 요구사항만 전달한다.

4. **실패 처리**: Skill 호출 결과가 명백히 실패(스킬 미존재, 에러) → `fallback` 경로(`references/phase-guides.md#review`) inline 가이드로 자동 전환. 채팅에 명시: "외부 스킬 실패 → fallback 가이드로 진행".

5. **결과 분기** (QA와 동형):
   - **CLEAN** (Critical/Important 0건) → state.json `completed_phases += "review"` → Phase 7 (Lesson)
   - **Minor만 존재** → lesson에 기록하고 통과로 처리 (진행을 막지 않음)
   - **FAIL** (Critical/Important ≥1) → state.json:
     ```json
     {
       "current_phase": "blocked",
     ...
     사용자에게:
     ```
     Review 지적 (Critical/Important). 다음 중 선택:
     (a) Execute 재진입 — 지적 수정 후 재실행
     (b) Lesson 기록 후 종료 — 현재 상태를 학습 기록으로 남기고 중단
     ```

6. **사용자 확인** — **[승인 4]** (CLEAN/Minor-only인 경우, 파이프라인 마지막 게이트):
   ```
   Review 통과 (Critical/Important 0건). Lesson 기록 단계로 진입할까요?
   ```
...
5. **최종 요약 출력**:
   ...
   Tier: 1 / Complexity: 3
   Phases: classify → brainstorm → plan → execute → qa → review → lesson
   Safe tag: safe/exec-...
