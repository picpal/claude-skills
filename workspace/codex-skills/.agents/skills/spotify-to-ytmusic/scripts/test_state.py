...
def test_default_data_home_uses_codex_directory(monkeypatch):
    monkeypatch.delenv("SPOTIFY_TO_YTMUSIC_HOME", raising=False)
    assert state.data_home() == state.Path.home() / ".codex" / "spotify-to-ytmusic"
