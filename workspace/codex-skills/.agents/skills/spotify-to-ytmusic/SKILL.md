...
description: Use when 사용자가 Spotify 플레이리스트 URL을 주며 YouTube Music으로 복제·이전·동기화해 달라고 하거나 "spotify to ytmusic", "플레이리스트 옮겨줘", "스포티파이에서 유튜브뮤직으로" 같은 요청을 한다.
---
...
```bash
python3 -m pip install -r "<스킬 디렉토리>/requirements.txt"
```
...
미설정이면 셋업 절차가 그대로 출력된다. 요약하면: music.youtube.com에 로그인한 브라우저의 개발자 도구에서 `/youtubei/v1/` POST 요청의 Request Headers를 복사 → `ytmusicapi browser --file ~/.codex/spotify-to-ytmusic/browser.json` 실행 후 붙여넣기.
...
| 용도 | 경로 |
|---|---|
| YT 인증 | `~/.codex/spotify-to-ytmusic/browser.json` |
| 동기화 상태 | `~/.codex/spotify-to-ytmusic/state/<playlist_id>.json` |
| 리포트 | `~/.codex/spotify-to-ytmusic/reports/<이름>-<날짜시각>.md` |
