...
2. **환경 결정** (아래 결정표 4개를 spec 작성 전에 전부 확정). **세션 확보 경로를 코드에서 찾지 못했으면 여기서 멈춘다** — 시드 계정·로그인 API·세션 주입 경로 중 하나를 확보하거나 사용자에게 물은 뒤 3단계로 간다. 하네스를 먼저 만들고 뒤늦게 발견하면 `dependencies` 때문에 파일럿 spec까지 함께 막힌다.
...
| 인증 | 폼 로그인: **기동 스크립트가 심는 시드 계정**(자격증명 단일 출처)으로 setup 프로젝트에서 UI 로그인 1회 → `storageState` 저장, 본 프로젝트가 `dependencies`로 상속 | OAuth/SSO·MFA 강제: 테스트 계정으로 storageState 생성이 가능한지 **spec 작성 전에** 확보. 불가하면 세션 주입 API 등 테스트 전용 경로가 선행 조건. 시드·로그인 API를 코드에서 못 찾으면 **추측하지 말고 묻는다** — 확보도 답변도 없으면 작업을 멈추고 보고 |
