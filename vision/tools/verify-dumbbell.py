...
2. NON-TEXT CONTRAST - the solid endpoint and the connector are the marks that
   ...
   joining the pair. Accent-on-paper is 5.02:1 in this fork, but the rule is kept: shape and
   boundary redundancy must not depend on the accent token's current value,
...
Usage:
    python3 tools/verify-dumbbell.py
    python3 tools/verify-dumbbell.py --reference path/to/type-bar.md
...
REFERENCE = ROOT / "references/type-bar.md"
