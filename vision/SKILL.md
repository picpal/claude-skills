...
metadata:
  version: "1.0.0"
  forked-from: "cathrynlavery/diagram-design v2.6"
---
...
> **Fixing a defect, or editing this skill? Read [`lessons.md`](lessons.md) first.**
> It records every bug found in this fork, why it survived the checkers, and the
...
Open [`references/style-guide.md`](references/style-guide.md) and check the default tokens. If they're still the shipped defaults (paper `#f5f5f5`, ink `#2d3142`, accent `#0f766e` deep-teal), **pause and ask the user**:

> *"This is your first diagram in this project. The style guide is still at the default (neutral white-smoke + deep-teal). Do you want to customize it to match your brand first? Options: (a) pull from your website URL, (b) extract from an installed skill, (c) extract from a local folder / design-system directory, (d) paste tokens manually, (e) proceed with the default for now, (f) load a saved client profile."*
...
| JetBrains Mono as blanket "dev" font | Mono is for *technical* content — ports, commands, URLs. Names go in Pretendard. |
...
- **Title** — Pretendard, 1.75rem, 600 — H1 only
- **Node name** — Pretendard (sans), 12px, 600 — human-readable labels
...
- **Editorial aside** — Pretendard *italic*, 14px — callouts only
...
**Never add `family=Pretendard` to that URL.** Pretendard is not on Google Fonts — the request 404s and the page silently falls back, but `lint-skin.py` trusts the URL's family list and reports no finding, so the failure is invisible. Pretendard also cannot be `@import`ed, self-hosted, or embedded as a data URI (every non-fragment CSS `url()` is rejected). Name it first in the stack and let it resolve from a local install; `Noto Sans KR` above is the loaded fallback and covers Hangul either way. Full rationale in `references/style-guide.md` § Font stack.

**Korean / CJK labels.** Pretendard and Noto Sans KR both cover Hangul, so `node-name` needs nothing extra. Mono does: Geist Mono has no Hangul, so any sublabel, tag, or arrow label containing Korean must use `font-family="'Geist Mono', 'Noto Sans Mono CJK KR', monospace"`. The "≤14 characters, all-caps" arrow-label rule assumes Latin — Hangul has no case, so cap Korean arrow labels at **6 glyphs** and widen the mask rect to `glyphs × 8 + 12` (a full-width glyph advances ~1em, roughly double a Latin character at the same size).
...
- Icon set (server, DB, bucket, query, schedule, dashboard, notebook, …) → [primitive-icons.md](references/primitive-icons.md); vendor logos live separately in [primitive-icons-brand.md](references/primitive-icons-brand.md). Browse the gallery at [`assets/icons.html`](assets/icons.html), regenerated by `python3 tools/build-icons.py`.
  **Do not read either file whole** — the function set is ~10k tokens. Both files' names are listed in primitive-icons.md → Index (~0.4k, generated by `build-icons.py`); pull one icon with `grep -A6 '^### <name>' references/primitive-icons.md`.
  **The icon says what the node does; the label says which product does it** — reach for a brand logo only when the vendor is the subject, and never mix stroked and filled marks in one diagram. See style-guide.md → Icons.
...
<marker id="arrow-link" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#4c1d95"/>
</marker>
```
...
| Accent | teal `#0f766e` | Primary / highlighted / headline |
| Link-violet | `#4c1d95` | HTTP/API calls, external systems |
...
<!-- 4. Node name (Pretendard — human-readable) -->
<text x="CX" y="CY+2" fill="#2d3142" font-size="12" font-weight="600"
      font-family="'Pretendard', sans-serif" text-anchor="middle">Node Name</text>
...
| Gap between nodes | 20, 24, 32, 40, 48 — **never 0**; two boxes must not share a border |
...
**Siblings on the same row take a uniform pitch = node width + gap.** Compute the
row once (`n×width + (n−1)×gap`) and centre it in the viewBox; don't place each
group independently, or the last pair ends up sharing a border while the rest
have 20px. Types with their own gutter (treemap 4px, medallion 16px, data-flow
12px, kanban 12px vertical) say so in their reference. Verify with
`python3 tools/verify-spacing.py <file>` — it fails on any two node boxes that
touch or overlap on either axis.
...
1. **Header** — eyebrow (Geist Mono), title (Pretendard), optional subtitle (Pretendard muted).
...
**Verify the file, don't trust the eye** — from a skill checkout:
...
python3 tools/verify-icons.py     <file>   # 24-grid, icon vs label axis, mixed icon styles
```
...
- [ ] **No label mask overlaps a node drawn after it? (Node fill would clip the text — §6 rule 6. Run `python3 <skill-root>/tools/verify-geometry.py <file>`.)**
