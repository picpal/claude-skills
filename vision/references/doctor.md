...
- **Maintainer-checkout mode**: use this only when the resolved installation
  ...
  `verify-plugin-package.py` (upstream CI only — not shipped in this fork). Add the repository integrity checks below.
...
- Verify these repository scripts exist:
  - `verify-drawio-import.py` (upstream CI only — not shipped in this fork)
  - `verify-mermaid-import.py` (upstream CI only — not shipped in this fork)
  - `tools/lint-skin.py`
  - `tools/verify-geometry.py`
  - (`verify-motion.py` is not shipped in this fork)
  - `verify-docs-sync.py` (upstream CI only — not shipped in this fork)
...
- `[FAIL] Missing verify-docs-sync.py (upstream CI only)`
...
[PASS] verify-drawio-import.py (upstream CI only) present
...
