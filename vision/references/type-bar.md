...
- **X-axis labels:** centered below each bar, Pretendard 11px 600 for category names.
...
Focal bar: replace fill with `rgba(15,118,110,0.12)`, stroke with `#0f766e`, label fill with `#0f766e`.
...
- **Plot area:** left margin 200px for row labels — this replaces the 80px margin above — so `x` 200→960, `y` 40→420 inside `0 0 1000 500`. Row labels right-aligned at x=188, Pretendard 11px 600 ink (the category-label size this type already uses).
...
**Non-text contrast: the boundary carries it, not the fill.** Accent on paper is 5.02:1 in this fork (it was 2.86:1 upstream, under the 3:1 WCAG 1.4.11 asks of a graphical object needed to understand the content). The rule is kept regardless: a mark's legibility must not depend on the accent token's current value, and shape redundancy does not waive 1.4.11 either, because the reader still has to see the mark's edge and the line joining the pair. So neither is left to the accent: the solid endpoint takes a 1px `ink` stroke (11.82:1 against paper, 4.13:1 against its own fill) and the connector takes 55% ink on light, 40% on dark (3.19:1 and 3.24:1). The hollow end already cleared it via its `muted` stroke at 6.11:1 light and 7.07:1 dark. `tools/verify-dumbbell.py` asserts all four, so a later tint change cannot quietly drop one under the line. The hollow/solid shape difference stays as redundant encoding for greyscale and colour-vision deficiency — do not collapse the two dots to a single fill.
...
<circle cx="740" cy="96" r="6" fill="#0f766e" stroke="#2d3142" stroke-width="1"/>
...
<text x="188" y="100" fill="#2d3142" font-size="11" font-weight="600" font-family="'Pretendard', 'Noto Sans KR', sans-serif" text-anchor="end">Platform</text>
```
...
Two of these rules live in a formula rather than a drawing, so they are executable: `tools/verify-dumbbell.py` resolves the domain over every sign case and asserts finite coordinates and 3:1 marks, and `test-verify-dumbbell.py` (upstream CI only — not shipped in this fork) exercises both polarities — including all-zero and zero-touching data, and the sub-3:1 treatments this replaced. The checker reads the tokens out of this file, so prose and thresholds cannot drift apart. A shipped dumbbell example would additionally owe a check of drawn positions against the values printed at them.
