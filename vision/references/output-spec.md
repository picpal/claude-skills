...
|---|---|---|---|
| Title (Pretendard) | 28 | 40 | 32 |
| Node name (Pretendard 600) | 12 | 16 | 12 |
...
Pretendard covers Latin and Hangul, so **Korean labels need no special handling** — the default `node-name` stack already renders them. Japanese and Chinese are not covered; extend the family on those `<text>` elements only, and don't swap the whole skin:

```svg
<text font-family="'Pretendard', 'Hiragino Sans', 'Noto Sans JP', 'Yu Gothic', sans-serif">認証サービス</text>
<text font-family="'Pretendard', 'Noto Sans KR', sans-serif">인증 서비스</text>
```

Note that Pretendard itself cannot be linked as a webfont (see `style-guide.md` § Font stack), so on machines without it installed Hangul falls back to the loaded `Noto Sans KR`. Both are covered by the default stack; the Japanese stack carries no Hangul, so don't reuse it for Korean. For mono sublabels use `'Geist Mono', 'Noto Sans Mono CJK JP', monospace` (Japanese) or `'Geist Mono', 'Noto Sans Mono CJK KR', monospace` (Korean). Budget **1em per full-width CJK glyph**, not a small percentage over the average Latin glyph; `verify-treemap.py` uses that conservative contract for Unicode wide/full-width characters and treats combining marks as non-advancing. Prefer 12px names over 8px sublabels for CJK, which goes muddy below 10px. Actual width still varies by fallback font, so run the relevant geometry verifier after translating labels.
