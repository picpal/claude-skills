...
If the site has only one family, keep the schematic defaults for the missing roles (Pretendard for title, Geist Mono for mono). Don't force-pick a mono font that isn't on the site.
...
**From Markdown tables:** look for rows with hex values (`#rrggbb`) adjacent to role-like words. A row like `| accent | #0f766e |` maps directly.
...
```json
{ "colors": { "brand": { "primary": { "value": "#0f766e", "type": "color" } } } }
```
