...
```bash
# NOTE: this fork ships only lint-skin and verify-geometry under tools/.
# verify-motion.py / test-verify-motion.py are NOT included — check motion output by hand.
python3 tools/lint-skin.py path/to/animated-diagram.html
python3 tools/verify-geometry.py path/to/animated-diagram.html
```
