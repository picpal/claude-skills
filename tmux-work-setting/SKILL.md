...
1. **Base workspace 경로 결정** — 현재 작업 디렉토리(`$PWD`)를 기본값으로 제안하되, AskUserQuestion으로 사용자에게 변경 기회를 준다:
   - 현재 경로 사용 (기본값)
   - 직접 경로 입력 (Other 옵션으로)

2. **하위 디렉토리 탐색 및 분기 처리**:
   - base 경로 아래 `ls -d */`로 하위 디렉토리 확인
   - **디렉토리가 있는 경우** → 번호 목록으로 표시:
     ```
     📂 ~/Desktop/workspace 하위 프로젝트:
       ...
       3. my-api-server
     ```
   - **디렉토리가 없거나 프로젝트로 보이지 않는 경우** → 목록 표시를 건너뛰고, 사용자에게 각 pane의 프로젝트 경로를 절대경로로 직접 입력받는 모드로 전환. 안내 메시지:
     ```
     📂 {base_path} 아래에 하위 프로젝트가 없습니다.
     각 pane에 할당할 프로젝트의 절대경로를 직접 입력해주세요.
     ```

3. **각 pane에 프로젝트 할당** — 윈도우/pane별로 프로젝트를 선택받음:
   - 번호 입력 → 목록에서 선택 (목록이 있는 경우)
   - 절대경로 입력(`/`로 시작) → 그대로 사용
   - 이름 입력 → base path에 연결
   - **입력된 경로가 존재하지 않으면 즉시 알려주고 재입력 요청**
